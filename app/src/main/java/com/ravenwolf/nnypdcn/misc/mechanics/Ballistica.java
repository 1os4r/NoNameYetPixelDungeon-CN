...
//        trace_tmp[distance_tmp++] = cell;
    ...
    public static boolean castToMaxDist(int from, int to, int maxDist) {
        return castToMaxDist(from, to, maxDist, false);
    }
    ...
    public static boolean castToMaxDist(int from, int to, int maxDist, boolean hitChars) {
        ...
        boolean hit = Ballistica.calcMaxDist(from, to, hitChars, stepA, stepB, dA, dB, dA / 2, maxDist);
        ...
        if ( !hit ) {
            for (int err = 0; err <= dA; err++) {
                hit=Ballistica.calcMaxDist(from, to, hitChars, stepA, stepB, dA, dB, err, maxDist);
                if (hit) {
                    distance = distance_tmp;
                    trace = trace_tmp.clone();
                }
    ...
    public static boolean calcMaxDist(int from, int to, boolean hitChars,
