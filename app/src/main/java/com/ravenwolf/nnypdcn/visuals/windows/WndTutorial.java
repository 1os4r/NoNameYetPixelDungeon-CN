    ...
    private static final String[][] TXT_POINTS = {
            {
                    ...
                    "在屏幕最右侧有三个操作按钮，分别是格挡按钮(只有装备盾牌和近战武器时才会显示，长按则会拿起盾牌猛击敌人)，远程攻击按钮(只有装备了投武或具有相应箭矢的弓弩时才会显示，点击即可进行一次射击)，以及法杖攻击按钮(只有当你装备了魔杖时才会显示，点击即可进行一次施法)。",
            ...
            },
            {
                    ...
                    "所有远程攻击的实际命中率都会因攻击距离的增加而每格减少1/8。唯一的例外便是魔杖。并且其命中率由魔能属性决定。",
                    ...
                    "主动探索或点亮油灯可以确保你对周围的隐藏环境(如陷阱和隐藏门)一览无遗。需要注意的是，在地牢中点亮灯光会让敌人更轻易的注意到你的存在。",
                    "你的意志属性能够显著影响法杖的充能速度，并且影响在装备未识别物品时，察觉到诅咒并防止装备的几率。魔能属性影响法杖的效果和伤害，同时也会影响部分攻击性卷轴的效果。",
        
            },
            {
                    "近战武器被分为多种类别。其中最基本的类别就是重型单手武器，它们没有任何特殊效果或减益，并且与其他装备搭配使用时不会获得额外的力量惩罚。作为副手武器与主手的远程武器搭配时，在近战时，允许使用它进行近战攻击，而不是使用在近战中有额外惩罚的远程武器进行攻击",
        ...
        /*    "Most equipment has condition level. It slowly decreases as the item is used, but can be restored with the corresponding repair tools or scrolls of Upgrade. Every condition level affects item performance just as much as upgrade level does, but it doesn't affects the strength requirement of the weapon or the number of charges of the wand.",*/
                    ...
                    "最易于使用且数量充足的治疗方式就是你的水袋。喝掉水袋中的水时，你会恢复部分生命值（根据以损失生命值来恢复），并且可以在地牢中偶尔出现的水井里补充水袋。每个章节都至少拥有一口水井。",
                    ...
                    "如果发现背包格太少，可以在商店里购买包裹。不同的药草、药剂、卷轴或魔杖分别可以被收纳在一个专属的包裹中。此外，它还能保护这些物品免受环境的影响（比如火、冰等)。",
            },
            {
                    ...
                    "那么，这就是全部了。如果你从头到尾阅读完了这篇教程，那这就意味着你已经具备了游玩这款游戏所需的全部知识。一些更加细节的内容会在游戏加载界面出现(还请多多注意)，你也可以通过阅读像素地牢wiki上NNYPD的部分文章来了解更多游戏的机制。祝你好运，注意小心那些拟型怪！",
